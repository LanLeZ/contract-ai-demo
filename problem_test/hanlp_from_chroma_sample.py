...
输出：
- problem_test/hanlp_results/hanlp_dep_complexity_chroma_*.jsonl
"""
...
from backend.app.services.vector_store import VectorStore  # type: ignore
...
def _fetch_docs_from_chroma(
    *,
    ...
    vs = VectorStore(persist_directory=persist_dir, collection_name=collection_name)
    ...
    # 直接从 collection 取 documents/metadatas（不做 query）
    got = vs.collection.get(where=where, include=["documents", "metadatas"], limit=int(limit))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Chroma persist dir（默认读环境变量 CHROMA_PERSIST_DIR 或 ./chroma_db）",
    )
