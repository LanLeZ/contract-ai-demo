...
数据格式（JSONL），每行一个查询样本，例如：
    {
        "id": "q1",
        "query": "出租人能否单方面提高房租？",
        "relevant_chunk_ids": [
            "3-民法典/民法典（2021-01-01）.md#114"
        ]
    }

说明：
- query：模拟真实业务场景中的检索问题
- relevant_chunk_ids：认为"正确答案"的 chunk_id 列表，
  与 chunks JSONL 文件中的 "id" 字段对应。
- 在同时包含多个法律的统一语料下（例如 LawBench 涉及的多部法律），
  评测时会先根据 chunk_id 中的文件前缀（法律文件）过滤，只在对应法律内部
  判断是否命中正确条文，避免不同法律中同号条文（如“第一百四十条”）混淆。

用法示例：
    python -m embedding-test.eval_retrieval \\
        --model-npz embedding-test/embeddings/law_book_text-embedding-v3.npz \\
        --dataset embedding-test/eval/queries.jsonl \\
        --top-k 5
"""
...
def load_query_dataset(path: Path) -> List[QuerySample]:
...
def evaluate_retrieval(
    ...
    业务逻辑：
    - 评测 embedding 模型在检索任务上的表现，使用 Recall@k 和 MRR 指标
    - 确保查询向量化使用的模型与文档向量化使用的模型一致（通过模型识别自动匹配）
    - 在多法律统一语料下，先限制在对应法律内部，再判断是否命中正确条文
    - 通过 relevant_chunk_ids 标注判断检索结果是否正确
    
    技术实现：
    - 加载预计算的文档向量（.npz 格式）和查询数据集（JSONL 格式）
    - 对查询文本进行向量化，计算与所有文档的余弦相似度
    - 按相似度排序，找到第一个命中 relevant_chunk_ids 的排名
    - 计算 Recall@k（前 k 个结果中是否包含正确答案）和 MRR（平均倒数排名）
    
    评测指标说明：
    - Recall@k: 前 k 个检索结果中包含正确答案的查询比例
    - MRR (Mean Reciprocal Rank): 所有查询的倒数排名平均值，反映首次命中位置
    
    Args:
        model_npz: 预计算的文档向量文件路径（.npz 格式）
        dataset_path: 检索评测数据集路径（JSONL 格式）
        top_k: 计算 Recall@k 中的 k 值（默认 5）
    """
    ...
    # 3. 加载检索数据集
    samples = load_query_dataset(dataset_path)
    print(f"📚 检索样本数: {len(samples)}")
    ...
    for idx, sample in enumerate(samples, 1):
        print(f"\n[{idx}/{len(samples)}] 查询: {sample.id} - {sample.query}")
    ...
    if output_path:
        ...
        default_output = model_npz.parent / f"eval_retrieval_{model_name.replace('/', '_')}_n{n}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="检索准确率评测脚本")
    parser.add_argument(
        "--model-npz",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=str(EVAL_DIR / "queries.jsonl"),
        help="检索评测数据集 JSONL 路径（默认 embedding-test/eval/queries.jsonl）",
    )
    parser.add_argument(
        "--top-k",
        type=int,
